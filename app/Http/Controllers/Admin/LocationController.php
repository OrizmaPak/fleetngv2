...
class LocationController extends Controller
{
  ...
  {
    if ($request->ajax()) {
      if(session('user_role')==$this->allowsuperadmin)
      {
        $data = PickupLocation::with('users')->orderBy('id', 'DESC')->get();
      }
      else
      {
        if(Auth::user()->user_type==2){ //admin
          $data = PickupLocation::with('users')->orderBy('id', 'DESC')->get();
      ...
      return Datatables::of($data)
        ...
        ->addColumn('company_user', function ($data) {
          ...
          {
            return '<a href="' . route('view-user', $data->user_id) . '" class="mr-1">'.(!empty($data->users->first_name)?$data->users->first_name:'') . ' ' . (!empty($data->users->last_name)?$data->users->last_name:'').'</a>';
          }
          else
          {
            return (!empty($data->users->first_name)?$data->users->first_name:'') . ' ' . (!empty($data->users->last_name)?$data->users->last_name:'');
          }
        ...
        ->rawColumns(['location', 'is_active', 'action','company_user','location_name'])
        ->make(true);
    }
  }
