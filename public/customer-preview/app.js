(function () {
  ...
  function shell(title, content) {
    const nav = [['overview','Dashboard','grid'],['payments','Payment','credit-card'],['profile','Profile','user']];
    const active = route.startsWith('payment-return') ? 'payments' : route;
  ...
  function tablePage() {
    ...
    const title = isDraft ? 'Draft bookings' : isPayment ? 'Payment' : 'My trips';
  ...
  async function render() {
    ...
    try {
      ...
      else shell('Not found',empty('Page not found','Return to your account dashboard.',linkButton('overview','Dashboard','arrow-left',true)));
    } catch (error) { root.innerHTML = '<main id="main">' + empty('Unable to load this page',error.message,linkButton('overview','Back to dashboard','arrow-left',true)) + '</main>'; icons(); }
  }
