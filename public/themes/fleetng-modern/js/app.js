(function () {
  ...
  function shouldIgnoreLoadingControl(control) {
    ...
    if (control.matches('[data-toggle], [data-dismiss], [data-density], [data-table-density], [data-table-page], [data-sort], [data-table-retry], [data-row-action], .dropdown-toggle, .close, .fleetng-password-toggle button, .fleetng-theme-toggle, .fleetng-nav-collapse, .fleetng-mobile-toggle, .fleetng-sort-button')) return true;
    if (control.closest('.fleetng-density-controls, .dataTables_paginate, .dataTables_length, .fleetng-table-pagination, .fleetng-password-toggle')) return true;
  ...
  function setControlLoading(control, label) {
    ...
    if (control.tagName === 'INPUT') {
      ...
      control.innerHTML = '<span class="fleetng-loading-spinner" aria-hidden="true"></span><span>' + loadingLabel + '</span>';
    }
  ...
  function initialiseLoadingStates() {
    document.addEventListener('submit', function (event) {
      ...
      var submitter = event.submitter || form.querySelector('button[type="submit"], input[type="submit"], .submitBtn');
